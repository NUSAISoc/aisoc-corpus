...
- **Formatting Standards**: Provide templates for LaTeX (inline `$` and block `$$`), syntax-highlighted code blocks, inline source links, and Further Reading (defining a required `furtherReading` array in frontmatter).
- **Automated Validation (PR Verification)**: Run CI checks on PRs verifying frontmatter structures, KaTeX compile success, Further Reading source URLs, and internal link integrity.
...
- **Tasks**:
  1. Define a strict Zod schema for topic files in `src/content/config.ts` (title, description, difficulty, domains, tags, prerequisites, furtherReading).
...
- **Tasks**:
  1. Write a local verification script (`scripts/validate-content.sh`) to perform static checks on frontmatter, LaTeX, Further Reading, and internal links.
