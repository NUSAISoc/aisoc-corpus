...
$$\Huge P(w|D) \propto P(D|w) \cdot P(w)$$
...
$$\Huge D = \left\{ (x_1, y_1), (x_2, y_2), ... (x_N, y_N) \right\}$$
...
$$\Huge (x_i, y_i) \sim P_{\text{true}}(x,y)$$
...
$$\Huge \mathcal{L}(w ; D) = P(d_1, d_2, \dots, d_N | w) = \prod_{i=1}^{N} P(d_i | w)$$
...
$$\Huge \mathcal{L}(w_A ; D) > \mathcal{L}(w_B ; D) \implies w_A \text{ explains the entire dataset better than } w_B$$
...
$$\Huge w_{MLE} = \arg\max_{w} P(D|w)$$
*(decomposed into each datapoint and with the log function:)*
$$w_{MLE} = \arg\max_{w} \sum_{i=1}^{N} \log P(d_i | w)$$
...
$$\Huge P(D|w) = \prod_{i=1}^N P(d_i|w) = \prod_{i=1}^{N} P(x_i, y_i | w)$$
...
$$\Huge P(D|w) = \prod_{i=1}^{N} P(y_i | x_i; w) \cdot P(x_i)$$
...
$$\Huge P(w|D) = \frac{P(D|w) \cdot P(w)}{P(D)}$$
...
$$\Huge P(w|D) = \frac{\left[ \prod_{i=1}^{N} P(y_i | x_i; w) \cdot P(x_i) \right] \cdot P(w)}{\int \left[ \prod_{i=1}^{N} P(y_i | x_i; w) \cdot P(x_i) \right] \cdot P(w) dw}$$
...
$$\small P(D) = \sum_{\mathbf{w} \in \Omega} \left[ \prod_{i=1}^{N} P(y_i | x_i; \mathbf{w}) \cdot P(x_i) \right] \cdot P(\mathbf{w})$$
...
$$P(w|D) = \frac{\left( \prod_{i=1}^{N} P(x_i) \right) \cdot \left( \prod_{i=1}^{N} P(y_i | x_i; w) \right) \cdot P(w)}{\left( \prod_{i=1}^{N} P(x_i) \right) \cdot \int \left( \prod_{i=1}^{N} P(y_i | x_i; w) \right) \cdot P(w) dw}$$

which cancels out to:

$$\frac{\left[ \prod_{i=1}^{N} P(y_i | x_i; w) \right] \cdot P(w)}{\int \left[ \prod_{i=1}^{N} P(y_i | x_i; w) \right] \cdot P(w) dw}$$
recalling the proportionality described earlier as part of the explanation of the Maximum A Posteriori:
$$\Huge P(w|D) \propto \int\left[ \prod_{i=1}^{N} P(y_i | x_i; w) \right] \cdot P(w) d(w)$$
...
$$\Huge \left\{ (x_1, z_1), (x_2, z_2), ... (x_N, z_N) \right\}$$
...
$$\Huge q(x_t | x_{t-1}) = \mathcal{N}(x_t; \sqrt{1 - \beta_t}x_{t-1}, \beta_t \mathbf{I})$$
...
$$\Huge q(x_{t-1} | x_t) = \frac{q(x_t | x_{t-1}) q(x_{t-1})}{q(x_t)}$$
